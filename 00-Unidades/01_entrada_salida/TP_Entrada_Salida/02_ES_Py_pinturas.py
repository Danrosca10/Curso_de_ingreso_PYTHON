...
class App(customtkinter.CTk):
    ...
    def btn_convertir_c_f_on_click(self):
        ...
        temperatura_c_numero= float(temperatura_c_texto)
        transformacion_a_f= (temperatura_c_numero *9/5) +32
        alert(title="transformacion a f", message=transformacion_a_f)

    def btn_convertir_f_c_on_click(self):
        temperatura_f_texto= self.txt_temperatura_f.get()
        temperatura_f_numero= float(temperatura_f_texto)
        transformacion_a_c= (temperatura_f_numero - 32) *5/9
        alert(title="trasformacion a c", message=transformacion_a_c)
